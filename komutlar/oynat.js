...
exports.run = async (client, message, args) => {
    ...
    if (!args[0]) return message.channel.send("Lütfen Dinlemek istediğin şarkıyı yazın! (Şarkı ismi veya Youtube URLsi)");
        
    if (!voiceChannel) return message.channel.send(`⚠️\`HATA\` Lütfen herhangi bir sesli kanala katılınız.`);

    var permissions = voiceChannel.permissionsFor(client.user);
    if (!permissions.has('CONNECT')) {
      return message.channel.send(`⚠️\`HATA\` Herhangi bir sesli kanala katılabilmek için yeterli iznim yok.`);
    }
  
  
    if (!permissions.has('SPEAK')) {
      return message.channel.send(`⚠️\`HATA\` Müzik açamıyorum/şarkı çalamıyorum çünkü kanalda konuşma iznim yok veya mikrofonum kapalı.`);
    }
      ...
      try {
          ...
          console.error(err);
          return message.channel.send(`⚠️\`HATA\` Aradığınız isimde bir şarkı bulunamadı!`);
        }
      }
...
serverQueue.textChannel.send(`<:yt:643482120463056896> **Aranıyor** :mag_right: \`${message.content}\``)
          
       const playingBed = new RichEmbed()
        .setColor("RANDOM")
        .setAuthor(`Şarkı Bulundu!`, song.thumbnail)
